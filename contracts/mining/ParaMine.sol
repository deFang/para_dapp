...
import {IParaRewardVault, ParaRewardVault} from "./ParaRewardVault.sol";


contract ParaMine is Ownable {
    ...
    address public paraRewardVault;
    ...
    constructor(address _paraToken, uint256 _startBlock) public {
        paraRewardVault = address(new ParaRewardVault(_paraToken));
    ...
    function setReward(uint256 _paraPerBlock, bool _withUpdate) external onlyOwner {
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    function getPendingReward(address _lpToken, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 ParaReward = block
                .number
                .sub(pool.lastRewardBlock)
                .mul(paraPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accParaPerShare = accParaPerShare.add(DecimalMath.divFloor(ParaReward, lpSupply));
        }
        return DecimalMath.mul(user.amount, accParaPerShare).sub(user.rewardDebt);
    }
    ...
    function getAllPendingReward(address _user) external view returns (uint256) {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            totalReward = totalReward.add(
                DecimalMath.mul(user.amount, accParaPerShare).sub(user.rewardDebt)
            );
        }
        return totalReward;
    }
    ...
    function getDlpMiningSpeed(address _lpToken) external view returns (uint256) {
        uint256 pid = getPid(_lpToken);
        PoolInfo storage pool = poolInfos[pid];
        return paraPerBlock.mul(pool.allocPoint).div(totalAllocPoint);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 ParaReward = block
            ...
            .mul(pool.allocPoint)
            .div(totalAllocPoint);
        pool.accParaPerShare = pool.accParaPerShare.add(DecimalMath.divFloor(ParaReward, lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        if (user.amount > 0) {
            uint256 pending = DecimalMath.mul(user.amount, pool.accParaPerShare).sub(
                user.rewardDebt
            );
        ...
        user.rewardDebt = DecimalMath.mul(user.amount, pool.accParaPerShare);
    ...
    function withdraw(address _lpToken, uint256 _amount) public {
        ...
        updatePool(pid);
        uint256 pending = DecimalMath.mul(user.amount, pool.accParaPerShare).sub(user.rewardDebt);
        safeParaTransfer(msg.sender, pending);
    ...
    function claimAll() public {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            ...
            updatePool(pid);
            pending = pending.add(
                DecimalMath.mul(user.amount, pool.accParaPerShare).sub(user.rewardDebt)
            );
            user.rewardDebt = DecimalMath.mul(user.amount, pool.accParaPerShare);
        }
        safeParaTransfer(msg.sender, pending);
    ...
    // Safe Para transfer function
    function safeParaTransfer(address _to, uint256 _amount) internal {
        IParaRewardVault(paraRewardVault).reward(_to, _amount);
